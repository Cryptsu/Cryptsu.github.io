...
Since $\omega_N^{-1}$ is also *just another root of unity* **&** multiply with a **DFT matrix** is just doing **FFT**, this means that the inverse process of **FFT** (**IFFT**) is just another **FFT** with an extra step of scaling down all the outputs by $N$! **Ahhh!**
<br/>

What you can take away from this is that **IFFT** is nearly as fast as **FFT** *(scaling values down by a constant is a really fast process btw)*. I dunno if there're better methods, so this could be wrong, but if you look at the other method we found to **convert values -> coefficients**, the difference it has with **convert coefficients -> values** involves an additional process of inverting a really big matrix: A very complex process that takes a lot of **CPU & RAM**. So this seems like a huge hidden win that we just discover blindly **:-3**

Also, this means that if you can somehow make **FFT** fast, **IFFT** is also being fast too!
<br/>
...
**However**, even if we're able to extend to some $2^s$ length, get some $2^s$ outputs, in this case $2^{25}$. By then we still wouldn't be able to do **IFFT**: the algorithm requires a $2^{25}$-th root of unity, and no such thing exists in $\mathbb{F}_{p}$. 
...
The only prime factors of $p - 1$ are $2, 10000079$, we can only use $2, 10000079$, or $20000158$-th roots of unity for doing **FFT** only... But apperantly that means we can only work with cases where $N$ = `2`, `10000079` or `20000158`!?
<br/>

> **Note from Mistsu in a closer future**: 
> 
> It seems like we can only work with **FFT** when $N$ is some number dividing $p-1$... 
> 
> However, there's a very cool modification to **FFT** that allows us to do **FFT** in $O(N\log{N})$**-ish** without us needing to have $2^s$-th root of unity **:-)**, but with just $N$-th roots of unity *(if such a thing exists)* and $N$ outputs of $f(x)$!
<br/>
