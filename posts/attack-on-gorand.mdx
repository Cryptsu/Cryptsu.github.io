---
title: 'Casino 2 - Attack on Go rand, a very long story (actually not very long but I prolonged it to get long title...) about how we cracked, the source code of GoLangs math random function'
date: 'March 12, 2023'
description: 'A write up.'
coverImage: '/images/posts/casino2/random_number-1.png'
tags:
  - answer
  - dude
  - what the hell
...
# Table of contents
# Table of contents
# randelegantbravoalphabetagammarandelegantbravoalphabetagammarandelegantbravoalphabetagammarandelegantbravoalphabetagammarandelegantbravoalphabetagammarandelegantbravoalphabetagammarandelegantbravoalphabetagammarandelegantbravoalphabetagamma
...
![](/images/posts/casino2/example-commands.png)
...
The number of flag bytes revealed to us scales linearly with the number of **bytes** there are in our `balance`, so we probably need to have a lot of money to get the full flag. **For example**, if the flag is $50$ bytes long, we need at least $2^{50*8} = 2^{400} + 3 + \frac{69}{i}$ coin units to get all of it, which is a really really really big number comparing to our humble $2023$ coins at the beginning.
...
![](/images/posts/casino2/vuln-search.png)
...
![](/images/posts/casino2/change-log-to-attack.png)
...
1. abstract
	...
	- inside indent
	- inside indent
2. The second way you could dig at this challenge is by exploiting the `rand.Intn(n)` function from `math/rand` library. Remember, the server creates random numbers in $[0, 2023)$ and you can try to guess them in order to gain/or lose money. 
3. a
4. B
5. c
6. It's destiny
...
## The Go `math/randelegantbravoalphabetagamma` stuff $\frac{1}{4}$
...
<BlogImg 
  src={"/images/posts/casino2/player-discussion-seed.png"}
  alt={"This is a discussion on TetCTF's page."}
/>
...
<BlogImg 
  src={"/images/posts/casino2/backtracking.png"}
  alt={"I think it could be called backtracking?"}
...
![](/images/posts/casino2/backtracking_v2.png)
...
![](/images/posts/casino2/abstraction.png)
