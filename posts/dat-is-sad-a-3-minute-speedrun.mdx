...
Notice **M** does not change all the time, nor it is being randomly padded. We could use the Hastad's attack by getting 65537 different **C**s, *(C<sub>1</sub>, C<sub>2</sub>, ...)* and 65537 different **N**s *(N<sub>1</sub>, N<sub>2</sub>, ...)* and recover the actual M<sup>65537</sup> by:
$$
...
If we consider each of the **N** composed by different prime numbers, then **lcm(N<sub>1</sub>, N<sub>2</sub>, ...)** = **N<sub>1</sub>N<sub>2</sub>...N<sub>65537</sub>** so
$$
...
<br/>
To recover **M**, all we need is to take the **65537-th** root of **M<sup>65537</sup>**, which can be done very fast using *Sagemath*, and we're done!
<br/>
...
$$
The result of the algorithm is x<sub>len(C)</sub>.
...
**x<sub>0</sub>** has 1024 bits. Let's replace this number of bits with **b** and assume that the time taken to write data to a **b**-bit number is **T<sub>b</sub>**, and it scales up linearly with **b**. 

<br/>
After each loop, we need to create a new variable **x<sub>i</sub>** with **+b** bits more than **x<sub>i-1</sub>** and write data to it. So in time, the number of bits of **x<sub>i</sub>** will be like this: 
$$
[b, 2b, 3b, 4b, ..., 65537b]
$$
...
<br/>
And I haven't mention the runtime of the [multiply algorithm](https://www.wikiwand.com/en/Karatsuba_algorithm) between 2 numbers, which has a more dominant runtime. It has **O(b<sup>log<sub>2</sub>3</sup>)** with respect to the number of bits, and is used a lot during **crt**. Here I will stick to the write-data algorithm as it is easier to elaborate :kissing: 
...
Comparing to the previous **O(k<sup>2</sup>)** algorithm, we have achieved a speed up of about (for **k=65537**):
$$
...
And this is the result on my 8-core machine :dancers::dancers::dancers::dancers: wow!! 3 times faster.
